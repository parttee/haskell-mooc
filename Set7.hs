...
member _ (Set []) = False
member e (Set xs) = if head xs == e then True else member e (Set (tail xs))

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add e es@(Set xs) = if member e es then es else Set (sort (e:xs))
...
data State = Start | AddedEggs | AddedEggsSugar | AddedEggsFlour | AddedEggsSugarFlour | Mixed | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = AddedEggs
step AddedEggs AddSugar = AddedEggsSugar
step AddedEggs AddFlour = AddedEggsFlour
step AddedEggsSugar AddFlour = AddedEggsSugarFlour
step AddedEggsFlour AddSugar = AddedEggsSugarFlour
step AddedEggsSugarFlour Mix = Mixed
...
average (x :| xs) = (sum all) / fromIntegral (length all)
                    where all = x:xs
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  (Set x) <> (Set y) = Set (sort (x `union` y))
...
passwordAllowed pwd (MinimumLength x) = length pwd >= x
passwordAllowed pwd (ContainsSome (x:[])) = elem x pwd
passwordAllowed pwd (ContainsSome (x:xs)) = if elem x pwd then True else passwordAllowed pwd (ContainsSome xs)
passwordAllowed pwd (DoesNotContain (x:[])) = notElem x pwd
passwordAllowed pwd (DoesNotContain (x:xs)) = if elem x pwd then False else passwordAllowed pwd (DoesNotContain xs)
passwordAllowed pwd (And x y) = passwordAllowed pwd x && passwordAllowed pwd y
passwordAllowed pwd (Or x y) = passwordAllowed pwd x || passwordAllowed pwd y
