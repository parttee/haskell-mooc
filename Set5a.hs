...
data UpDown = UdUp Int | UdDown Int
...
tick :: UpDown -> UpDown
tick (UdUp ud) = UdUp (ud + 1)
tick (UdDown ud) = UdDown (ud - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UdUp ud) = UdDown ud
toggle (UdDown ud) = UdUp ud
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = let (r1:g1:b1:_) = rgb c1;
                      (r2:g2:b2:_) = rgb c2; 
                      avg x y = (x + y) / 2;
                  in [avg r1 r2, avg g1 g2, avg b1 b2]
rgb (Invert clr) = let (r:g:b:_) = rgb clr
                   in [1 - r, 1 - g, 1 - b]
...
toNat z
  | z < 0 = Nothing
  | otherwise = Just (loorpz z)
  where loorpz x
          | x <= 0 = Zero
          | otherwise = PlusOne (loorpz (x - 1));
...
fromBin' :: Bin -> Int -> Int
fromBin' End x = 0
fromBin' (O b) x = 0 + (fromBin' b (x + 1))
fromBin' (I b) x = 2 ^ x + (fromBin' b (x + 1))

toBin :: Int -> Bin
toBin x
  | x == 0 = O End
  | x == 1 = I End
  | fromIntegral x / fromIntegral 2 == fromIntegral intX = O (toBin intX)
  | otherwise = I (toBin intX)
  where intX = x `div` 2