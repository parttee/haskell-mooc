...
cut (Shape s1) (Shape s2) = Shape (\coord -> if (s1 coord) && (s2 coord) then False else (s1 coord))
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where f (Coord x y) = let allColors = [(p (Coord x (y - 1))),
                                          (p (Coord x (y + 1))),
                                          (p (Coord x y)),
                                          (p (Coord (x - 1) y)),
                                          (p (Coord (x + 1) y))];
                          in avgColor allColors (length allColors)
           
avgColor :: [Color] -> Int -> Color
avgColor [] m = Color 0 0 0
avgColor ((Color r g b):[]) m = Color (r `div` m) (g `div` m) (b `div` m)
avgColor ((Color r g b):(Color r2 g2 b2):xs) m = avgColor ((Color (r + r2) (g + g2) (b + b2)):xs) m
------------------------------------------------------------------------------
