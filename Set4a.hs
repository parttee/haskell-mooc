...
allEqual [] = True
allEqual (x:[]) = True
allEqual (x:next@(y:xs)) = if x /= y then False else allEqual next
...
distinct [] = True
distinct (x:[]) = True
distinct (x:xs) = case elem x xs of
  True -> False
  False -> distinct xs
...
longest :: Ord a => [[a]] -> [a]
longest xs =  head $ 
              -- sort by the first elements in the lists
              sortBy (\(x:xs) (y:ys) -> x `compare` y) $ 
              -- filter by using the first element of the sorted list
              filter (\x -> length x == longestLen) byLongest
              where byLongest = sortBy (\x y -> length y `compare` length x) xs
                    longestLen = length $ head byLongest
...
incrementKey k v = map (\(l,r) -> if l == k 
                                  then (l, r + 1) 
                                  else (l, r)) v
...
winner scores player1 player2
  ...
  where p1Score = Map.findWithDefault 0 player1 scores
...
freqs [] = Map.fromList []
freqs (x:[]) = Map.fromList [(x,1)]
freqs (x:xs) = Map.alter incrementFreq x (freqs xs) 

incrementFreq v = case v of
  Nothing -> Just 1
  Just v -> Just (v + 1)
...
transfer from to amount bank
  | Map.member from bank == False = bank
  | Map.member to bank == False = bank
  | amount < 0 = bank
  | amount > fromAmount = bank
  | otherwise = Map.alter (transferReduce amount) from $ Map.alter (transferAdd amount) to bank
  where fromAmount = Map.findWithDefault 0 from bank
        toAmount = Map.findWithDefault 0 to bank

transferReduce v x = case x of
  Nothing -> Nothing
  Just x -> Just (x - v)

transferAdd v x = case x of
  Nothing -> Nothing
  Just x -> Just (x + v)
...
maxIndex arr = 
  let sorted = sortBy (\(xk,xv) (yk,yv) -> yv `compare` xv) $ Data.Array.assocs arr
      largest = head sorted
  in fst largest
