...
interpreter [] = []
interpreter commands = consume [0, 0] commands

consume _ [] = []
consume (y:x:_) (cmd:cmds)
    | cmd == "up" = consume [y + 1, x] cmds
    | cmd == "down" = consume [y - 1, x] cmds
    | cmd == "left" = consume [y, x - 1] cmds
    | cmd == "right" = consume [y, x + 1] cmds
    | cmd == "printY" = [show y] ++ consume [y, x] cmds
    | cmd == "printX" = [show x] ++ consume [y, x] cmds
    | otherwise = consume [y, x] cmds