...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node _ lg sm) = 1 + (treeSize lg) + (treeSize sm)
...
treeMax Empty = 0
treeMax (Node n Empty Empty) = n
treeMax (Node n br1 br2) = max (max n (treeMax br1)) (treeMax br2)
...
allValues condition (Node n br1 br2) =  if condition n && allValues condition br1 && allValues condition br2 
                                        then True 
                                        else False
...
mapTree f (Node x br1 br2) = Node (f x) (mapTree f br1) (mapTree f br2)
...
cull val (Node n br1 br2)
  | val == n = Empty
  | otherwise = Node n (cull val br1) (cull val br2)
...
isOrdered (Node n br1 br2) =  if (allValues (<n) br1 && allValues (>n) br2) 
                              then isOrdered br1 && isOrdered br2 
                              else False
...
walk [] x = valAtRoot x
walk (x:xs) (Node n br1 br2) = case x of
  StepL -> walk xs br1
  StepR -> walk xs br2
...
set [] val Empty = Empty
set [] val (Node n br1 br2) = Node val br1 br2
set (x:xs) val (Node n br1 br2) = case x of
  StepL -> Node n (set xs val br1) br2
  StepR -> Node n br1 (set xs val br2)
...
search x (Node n br1 br2)
  | x == n = Just []
  | otherwise = case search x br1 of
                Nothing -> case search x br2 of
                            Nothing -> Nothing
