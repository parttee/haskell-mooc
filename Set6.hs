...
instance Ord Country where
  compare x y = compare (numbered x) (numbered y) where
      numbered Finland = 1
      numbered Norway = 2
      numbered Switzerland = 3
...
instance Eq RationalNumber where
  (RationalNumber p p2) == (RationalNumber q q2) = p * q2 == p2 * q
...
simplify (RationalNumber p q) = RationalNumber (p `div` nom) (q `div` nom)
  where nom = gcd p q
...
instance Num RationalNumber where
  (RationalNumber p q) + (RationalNumber p2 q2) = simplify (RationalNumber (p * q2 + q * p2) (q * q2))
  (RationalNumber p q) * (RationalNumber p2 q2) = simplify (RationalNumber (p * p2) (q * q2))
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 c = c
  stepMany x c = stepMany (x - 1) (step c)
